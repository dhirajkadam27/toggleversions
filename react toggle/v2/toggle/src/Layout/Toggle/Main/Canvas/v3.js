...
function Canvas() {
    ...
    if (resizing) {
      ...
      const updateElement = (element) => {
        if (element.name === rect.name) {
          return { ...element, x: newRect.x - offset.x , y:newRect.y- offset.y,width:newRect.width,height:newRect.height };
        }
        if (element.elements) {
          return { ...element, elements: element.elements.map(updateElement) };
        }
  ...
  }, [offset,setOffset,setScale, scale, dragging, startPoint, data,resizing, rect, originalRect,LeftGuideLine,BottomGuideLine,RightGuideLine,TopGuideLine,setLeftGuideLine,setTopGuideLine,setBottomGuideLine,setRightGuideLine]);
