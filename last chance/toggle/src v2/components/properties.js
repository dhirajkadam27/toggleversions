...
function Properties() {
  ...
  const { propertieswidth, setpropertieswidth,zoom } = useUtils();

  useEffect(()=>{
    document.querySelector('.zoom').querySelector('button div span').innerHTML = `${parseInt(zoom*100)}%`;
  },[zoom])
    ...
    <div style={{ "right": `-${propertieswidth}px` }} className='Properties'>
      <div className='top'>
        ...
        <div className='topequalbox'>
          <Select
            label="Zoom"
            defaultSelectedKeys={[`${zoom*100}%`]}
            className="zoom"
          >
            <SelectItem key={zoom*100+"%"}>{zoom*100+"%"}</SelectItem>
        ...
        <div className='topequalbox'>
          <CiPlay1 onClick={()=>{window.open('#/preview', '_blank');}} className='preview' />
        </div>
